...
program
  ...
  .option(
    "-b, --browser",
    "open up a browser and show generated static license summary file"
  )
  .option("--no-browser", "prevents the browser from popping open")
  .option(
    "-i, --info",
    "provides information about all direct dependency licenses"
  )
  .option(
...
if (program.outputFilePath && !program.generate) {
  console.warn(
    chalk.bgYellow.black(
      "Warning: Using --outputFilePath without -g (--generate) will not have any effect."
    )
  );
}
...
if (program.browser && !program.generate) {
  console.warn(
    chalk.bgYellow.black(
      "Warning: Using --browser without -g (--generate) will not have any effect."
    )
  );
}
...
(async function () {
  try {
    ...
    if (!packageFileExists && !composerFileExists) {
      console.error(
        chalk.red(
          "There is no dependency file (package,json or composer.json) in the current directory."
        )
      );
    ...
    if (!nodeModulesExists && !vendorFolderExists) {
      console.error(
        chalk.red(
          "There is no dependency folder (node_modules/ or vendor/) in the current directory."
        )
      );
    ...
    let dependencies = Object.keys(
      JSON.parse(content)[isPackageMode() ? "dependencies" : "require"]
    );
    if (isComposerMode())
      dependencies = dependencies.filter((d) => d.indexOf("/") >= 0);
    console.log(
      chalk.magentaBright("Found %d dependencies."),
      dependencies.length
    );
    ...
    for (let dependency of dependencies) {
      try {
        ...
        const descriptionFile = JSON.parse(
          fs.readFileSync(`./${dependencyFolder}/${sourceFile}`, "utf-8")
        );
        ...
        const licenseFile = LICENSE_FILENAMES.filter((f) =>
          dependencyFolderFiles.includes(f)
        );
        if (licenseFile.length > 0) {
          licenseFileCounter++;
          licenseItem["license"] = fs.readFileSync(
            `./${dependencyFolder}/${licenseFile[0]}`,
            "utf-8"
          );
        ...
        if (
          (isPackageMode() && descriptionFile.hasOwnProperty("repository")) ||
          isComposerMode()
        ) {
        ...
        if (program.verbose)
          console.warn(
            chalk.red('No file or download available for "%s"'),
            dependency
          );
        licenseItems.push(licenseItem);
      } catch (e) {
        if (program.verbose)
          console.warn(
            chalk.red('Did not find directory for "%s"'),
            dependency,
            e
          );
      }
    ...
    if (!program.verbose)
      spinner.succeed(chalk.magentaBright("Scanning node_modules/... done!"));

    console.log(
      chalk.magentaBright("The following licenses are used:"),
      [...new Set(licenseItems.map((i) => i.type))].filter((i) => i).join(", ")
    );
    console.log(
      chalk.magentaBright("Found %d license files"),
      licenseFileCounter
    );
    console.log(
      chalk.magentaBright("Downloaded %d license files"),
      licenseDownloadCounter
    );
    ...
    if (program.info) {
      const licenseInfo = licenseItems
        .map((i) => {
          return {
            module: i.module,
      ...
      for (let licenseType of Object.keys(licenseInfo)) {
        console.log(
          chalk.blueBright.bold(`${licenseType} (%d usages)`),
          licenseInfo[licenseType].length
        );
    ...
    if (program.generate) {
      ...
      console.log(
        chalk.cyanBright("Creating license HTML file (" + filePath + ")...")
      );
      let html = "";
